...
function renderBoard(){const unitSelected=battle.selected!==null&&cardType(battle.hand[battle.selected])==='unit';$$('.lane').forEach((el,i)=>{el.classList.toggle('ready',unitSelected);$('.foe',el).innerHTML=unitHtml(battle.enemyLanes[i]);$('.ally',el).innerHTML=unitHtml(battle.lanes[i]);const foeUnit=$('.foe .unit',el),allyUnit=$('.ally .unit',el);if(foeUnit)bindCardHold(foeUnit,battle.enemyLanes[i]);if(allyUnit)bindCardHold(allyUnit,battle.lanes[i]);$('.ally',el).onclick=e=>{if(allyUnit?._suppressCardClick){allyUnit._suppressCardClick=false;return}chooseLane(i)}})}
function chooseLane(i){if(battle.resolving)return;if(battle.selected===null)return toast('先选择一张手牌');if(cardType(battle.hand[battle.selected])==='spell')return toast('这张法术无需选线，直接锁定即可');battle.target=i;$$('.lane').forEach((x,n)=>x.style.outline=n===i?'2px solid #ffe28a':'');toast(`${battle.hand[battle.selected].name} → ${['左翼','中央','右翼'][i]}`)}
