...
第三关卡死记录：战斗结算期间锁定按钮会被禁用；若上一战在结算路径中直接结束，按钮来不及恢复。新战斗虽然创建了新的 `battle` 对象，但复用的 DOM 按钮仍保持 `disabled`，造成地图第三关第一回合无法继续。修复后 `startBattle()` 明确恢复按钮和战线轮廓，`renderBattle()` 每次按当前 `battle.resolving` 同步按钮状态。页面资源版本已提升到 `20260716d`，避免手机继续使用旧脚本。
